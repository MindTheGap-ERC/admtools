...
#### Auxiliary Functions ####
{
  ...
  transform.foss.occ=function(foss.occ,am){
    remove.me=will.be.destroyed(foss.occ, am)
  ...
  # returns a list of same length as foss occ.
  # Contains TRUE if the corresponding occurrence will be destroyed by a hiatus, FALSE if it will be preserved
  will.be.destroyed=function(foss.occ,am){
    remove.me=is.na(pointtransform(points=max(am$time)-foss.occ$hmin,xdep=am$time,ydep=am$height,depositionmodel = "age model")$height)
    return(remove.me)
  }
...
tax.original=sim.taxonomy(tree.original,beta=1)
...
plot(tax.original,tree.original)

tree.original.traits=sim.trait.values(init = 1,tree=tree.original,v=1)
