...
`admtools` comes with utilities to deal with depth-depth curves that are
used for correlation between sections. See the developer documentation
([`vignette("admtools_doc")`](https://mindthegap-erc.github.io/admtools/articles/admtools_doc.md))
...
Alternatively, you can use two age-depth models to construct depth-depth
curves via `adm_to_ddc`. This will construct depth-depth curves for the
...
`get_L_units`, `set_L_units`, `get_section_names` and
`set_section_names`. To reverse the direction of correlation, use
`flip_ddc`:
...
![](correlation_files/figure-html/unnamed-chunk-5-2.png)
