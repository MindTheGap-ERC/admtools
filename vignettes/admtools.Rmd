...
In contrast to its name, the _admtools_ package currently deals with time and height instead of age and depth. In this sense, the age-depth models are time-height models. To handle ages, use time before the present. To handle depths, use height below a point of reference (e.g. sediment surface).
...
str(my_adm)
```
This gives you the option to manually manifulate the fields (if you wish to).

You can plot `adm` objects via the standard `plot` function. Here, I highlight hiatuses in red.
